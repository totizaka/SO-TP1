...
int next_movement(Game_map *game, Player *player, unsigned short width, unsigned short height) {
    ...
    for (unsigned char dir = 0; dir < 8; dir++) {
        ...
        if (value < 1 || value > 9) continue; // solo celdas válidas
 

        // Me fijo las celdas adj de las que posiblemente me moveria
        //Prioriza no bloquiarse
        int freedom = 0;
        for (unsigned char d = 0; d < 8; d++) {
            int fx = new_x + dx[d];
            int fy = new_y + dy[d];
            if (fx >= 0 && fx < width && fy >= 0 && fy < height) {
                int fvalue = game->board[fy * width + fx];
                if (fvalue >= 1 && fvalue <= 9)
                    freedom++;
            }
        }

        float score = value + 0.5 * freedom;
